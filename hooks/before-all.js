...
module.exports = async function globalSetup() {
  ...
  const isApiOnlyRun =
    ...
    (cliArgs.includes('tests/api') &&
      !cliArgs.match(/tests\/(connector|hoodsly-hub|wrh-hub|wiks-hub)/)) ||
...
async function authenticateUser(browser, { baseURL, username, password, statePath, role }) {
  ...
  try {
    ...
    try {
      ...
      if (stateExists) {
        console.warn(
          `[globalSetup] WARNING: Re-authentication failed — using STALE auth state for ${role}.`
        );
        console.warn(
          `[globalSetup] ACTION REQUIRED: Update the password for ${username} in environments/.env.local`
        );
    ...
    console.error(
      '[globalSetup] TIP: For API-only runs use "npm run test:api" or set SKIP_AUTH=true'
    );
    throw err;
  } finally {
    await context.close();
