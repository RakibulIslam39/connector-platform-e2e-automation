...
Partners or Products. Instead, everything is managed inside a modern **Vue/React
Single Page Application** (SPA) embedded at:
...
> **Why this matters:** All previous code that targeted `#title`,
> `#acf-field_...`, `li[title='...']` (Select2), `#the-list tr`, `span.edit a`,
> etc. was completely wrong. Those selectors only exist in the native WP admin
> post editor — which is bypassed entirely by the SPA.
...
> Note: The "Generate" API Key button **IS present** in the SPA. It was missing
> from the native WP admin ACF view, causing confusion earlier.

### Tab 2 — Products

- Shows a table of assigned products (image + name/SKU + custom title + remove
  button)
...
- Each row also has: Custom Title textbox, Custom SKU textbox, Price Override
  table
...
- Partner table columns: Partner (name + URL) | Platform | Products |
  Environment | Status | Actions
- Actions per row: `a:has-text("Edit")` | `button:has-text("Duplicate")` |
...
**Changed:** Complete rewrite (twice — first pass used wrong ACF selectors;
final pass uses SPA selectors)

**Current state:** SPA-based placeholder/role selectors. No more `#acf-field_*`
IDs.
...
- Method calls aligned to new page object (e.g. `clickEditForProduct` instead of
  `hoverProduct` + `clickEditButton`)
- Ventilation/trim test data now falls back to known attribute names if not in
  JSON
...
- Removed broken `importProducts()` call (method doesn't exist in new page
  object)
