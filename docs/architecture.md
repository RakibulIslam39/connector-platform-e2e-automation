...
This framework uses a layered architecture where each layer has a single responsibility.
...
| `common/utils/` | Logger, waits, retries, data generation |
| `helpers/` | Domain logic (SKU generation, order routing, partner config) |
