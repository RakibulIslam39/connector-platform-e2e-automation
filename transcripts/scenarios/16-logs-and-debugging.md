...
**URL**:
`https://hoodslypartnersconnector3.kinsta.cloud/wp-admin/admin.php?page=connector-hub#/logs`  
**Prerequisite**:
...
**Purpose**: Verify filtering logs by action type (Product Sync vs Attribute
Sync).
...
- "Attribute Sync": all entries are Attribute Sync action (may be 0 if none
  exist)
...
**Purpose**: Verify the "View Details" action shows full log information
including request payload.
...
- Sub-text: "Select a partner and enter an order ID to inspect the live payload
  and validate SKU mappings."
...
**Purpose**: Verify that entering an invalid order ID shows an appropriate
error.
...
- Error message displayed: something like "Order not found" or "Invalid order
  ID"
