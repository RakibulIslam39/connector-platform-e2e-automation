...
**Business Logic**: The Master SKU is a concatenation of product and attribute codes used to identify orders in HoodslyHub.  
...
**Purpose**: Verify Size attribute values have correct SKU codes in the database.
...
### Expected Results
- Most Partner Colors have `—` (no SKU) as they're referenced by name, not SKU code
...
- These are mostly color and option attributes that don't contribute to master SKU
...
**Purpose**: Verify that when an order is placed on the partner site, the Master SKU is generated.
...
**Purpose**: Verify that orders in HoodslyHub can be validated by their Master SKU.
...
**Purpose**: Verify that updating an attribute value's SKU code affects new order SKUs.
...
**Purpose**: Verify the `helpers/sku-helper.js` utility generates correct SKU codes.
