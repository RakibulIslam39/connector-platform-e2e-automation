...
**Live Data Verified**: 17 Partners | 44 Products | 58 Attribute Types | 300
Attribute Values | 8 Catalogs | 353+ Sync Logs
...
See [00-live-exploration-notes.md](00-live-exploration-notes.md) for raw
observations from the live MCP browser exploration session.
...
- Attributes tab: 4 expandable accordion sections per partner (Colors 7
  selected, Ventilations, Trims, Sizes)
- FAQs & Shipping tab: 2 sub-tabs (FAQs, Shipping & Returns) with rich text
  editor entries
...
- Basic Info: Title, SKU, Status, Description (rich text), Dimensions (rich
...
- **Attribute Values**: 300 values, filter by type + "Missing SKU" filter (133
  missing)
- **Catalogs**: 8 color catalogs (USCD, TA Cabinetry, Hoodsly, Jarlin, Everly,
  Jim Bishop, Homemark, JSI)

#### 07 — Settings

- **General**: Auto-sync checkbox (✅ enabled) + Webhook Timeout (30s)
- **Products**: 3 sub-tabs — FAQs (empty), Shipping & Returns (3 policies),
...
- **Sync Logs**: Stats (today/success/failed/rate), Action+Status filters,
  Refresh, View Details modal, Delete
- **Partner Order Log**: Select Partner + Order ID → fetch live payload for SKU
  validation
...
- Attribute price overrides: full table of all product attributes with base and
