...
**URL**: `https://hoodslypartnersconnector3.kinsta.cloud/wp-admin/admin.php?page=connector-hub#/partners`  
...
**Purpose**: Verify the partners list page renders correctly with all required UI elements.
...
- Table columns: PARTNER (name + initials badge + website), PLATFORM (with icon), PRODUCTS (count), ENVIRONMENT, STATUS, ACTIONS
...
- Live test: searching "Rakibul" returns 2 partners: TestRakibulQA, RakibulQAPartner
...
### Expected Results
- Second filter has options: All Status, Active, Inactive (or All, Staging, Production)
...
- Form shows: Cancel and "Create Partner" buttons (both at top AND at bottom of form)

---

## SCENARIO-02-06: Add Partner form — all fields and validation

**Purpose**: Verify the Add New Partner form has all required fields and validates input.
...
*Required fields validated client-side by Vue/React (not HTML required attribute)
...
### Validation Behavior
- Submit without fields: shows asterisks (*) on required fields; "Session expired" toast may show from API
...
const apiKey = await page.locator('input[placeholder="API key will be auto-generated"]').inputValue();
...
**Purpose**: Verify both Cancel links navigate back to Partners list without creating a partner.
...
**Purpose**: Verify that an existing partner's data loads correctly in the edit form.
...
**Purpose**: Verify the Products tab in Partner edit shows assigned products and allows adding more.
...
- Panel shows two columns: Available products list (left) + currently selected (right)
...
const availablePanel = page.locator('[class*="available"], text="AVAILABLE PRODUCTS"');
```
...
**Purpose**: Verify the Attributes tab displays all 4 attribute categories with selection counts.
...
- Sub-text: "Select which attributes this partner can access. You can also set custom names and prices."
...
- When "Partner Colors" expanded:
  ...
  - "Filter by Catalog" dropdown with 9 options: All Catalogs, USCD, TA Cabinetry, Hoodsly, Jarlin, Everly, Jim Bishop, Homemark, JSI
...
**Purpose**: Verify the FAQs & Shipping tab has two sub-sections and allows creating FAQs.
...
- Clicking "Add FAQ" adds an inline form with:
  ...
  - Title: text input
  - Answer: rich text editor (WordPress TinyMCE with "Add media", Visual, Code tabs)
  - "Delete" button (removes the entry)
  - "Done" button (saves the entry)

### Expected Results — Shipping & Returns Tab
- Default state: "No shipping & return policies added yet." + "Add Policy" button
...
- "Copy": copies the API key to system clipboard (check clipboard value or toaster notification)
...
**Purpose**: Verify each row in the Partners list has the correct data and actions.
...
- Column 1: Partner initials avatar badge (colored circle with 2-letter abbreviation)
...
- Column 7: Actions — Edit button (pen/blue hover), Delete button (trash/red hover)

### Action Button Selectors
```js
// Edit button (title="Edit" or blue hover icon)
const editBtn = row.locator('button[title="Edit"], button.hover\\:text-primary');

// Delete button (title="Delete" or red hover icon)
const deleteBtn = row.locator('button[title="Delete"], button.hover\\:text-red');
```
...
- (With 17 partners and default 20-per-page, all fit on one page — no pagination needed)
