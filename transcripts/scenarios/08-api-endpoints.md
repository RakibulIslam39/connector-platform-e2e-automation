...
> **Base URL:** `https://hoodslypartnersconnector3.kinsta.cloud` **Auth:**
> `api-signature` header + `website` header **Spec Files:**
> `tests/api/products-api.spec.js`, `tests/api/attributes-mapping-api.spec.js`
...
The Connector Platform exposes REST API endpoints used by partner site plugins
to sync product data and manage orders. Auth requires two headers:
...
**Priority:** High **Spec:** `tests/api/products-api.spec.js` **Assertions
covered:**
...
**Priority:** High **Spec:** `tests/api/attributes-mapping-api.spec.js`
**Assertions covered:**
...
The `website` header tells the API which partner site is making the request —
this affects what products and colors are returned.
