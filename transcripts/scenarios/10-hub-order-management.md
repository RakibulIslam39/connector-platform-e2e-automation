...
> **URL:** `{HUB_BASE_URL}/wp-admin/edit.php?post_type=shop_order` **Page
> Object:** `pages/hoodsly-hub/order-management.page.js` (543 lines) **Spec
> File:** `tests/hoodsly-hub/order-management.spec.js` **Status:** Skeleton
...
HoodslyHub is the central Order Management System. It receives orders forwarded
from partner sites and routes them to fulfillment shops. The order management
interface is the most feature-rich page in the Hub.
...
- **Status update**: change order status (Production / Finishing / Shipped /
  Cancelled)
...
3. Verify Hub-specific menu items are present (e.g., "Orders", "Manual
   Placement")

**Expected Result:** Hub admin dashboard loads **Page Object:**
...
**Priority:** High **Pre-condition:** Order has valid US phone, email, shipper
name **Steps:**
...
2. Verify Hub creates `orderId-1` and `orderId-2` as separate line items or
   sub-orders
...
- Hub uses WooCommerce orders (`post_type=shop_order`) — shares WP admin order
  list structure but with custom columns/meta
- BOL generation is async — may need extended wait or polling
- Partner Source Name caching: status sync may be delayed by minutes after Hub
  update
