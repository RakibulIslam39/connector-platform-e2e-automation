...
**URL**:
`https://hoodslypartnersconnector3.kinsta.cloud/wp-admin/admin.php?page=connector-hub#/attributes`  
**Prerequisite**:
WordPress admin authenticated  
**Live Data** (verified 2026-06-02): 58 Attribute Types | 300 Attribute Values |
8 Catalogs
...
- Intro text: "Define the types of attributes your products can have (e.g.,
  Color, Size, Material)."
- "Add Type" button (top right)
- Filter dropdown: "All Attribute Types" | "Product Attribute Type" | "Partner
  Attribute Type"
...
5. Verify count: 4 types (Partner Colors, Partner Ventilations, Partner Trims,
   Partner Sizes)
...
- Currently only 4 types have this on: Partner Colors, Partner Ventilations,
  Partner Trims, Partner Sizes
...
await page
  .locator('input[placeholder="Auto-generated if empty"]')
  .fill('test-type');
...
**Purpose**: Verify the Attribute Values tab lists all values with filtering
options.
...
**Purpose**: Verify the "Missing SKU" filter correctly shows attribute values
that have no SKU code.
...
- Examples of missing SKU values: "Our Color Options" (color_options type), "Our
  Colour Options"
...
- Example: Size "30 x 36" has SKU "3036", contributing to master SKU like
  `VEN-DBC-3036-WHT-...`
...
await page
  .locator('input[placeholder="e.g., USCD, TA Cabinetry"]')
  .fill('New Catalog');
await page.locator('button:has-text("Create")').click();
...
**Purpose**: Verify that "Color" type attribute (specifically "Partner Colors")
...
- Each row has: checkbox, colored circle swatch, name, catalog, custom name,
  price columns
