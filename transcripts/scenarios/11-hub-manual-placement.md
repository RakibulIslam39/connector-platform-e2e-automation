...
> **URL:** `{HUB_BASE_URL}/wp-admin/admin.php?page=manual-placement` **Page
> Object:** `pages/hoodsly-hub/order-placement.page.js` (367 lines) **Spec
> File:** `tests/hoodsly-hub/order-placement.spec.js` **Status:** Skeleton
> exists — needs real UI automation

---

## Feature Overview

Manual Placement is a Hub-specific workflow for orders that cannot be
auto-routed. Orders land here when:
...
**Priority:** High (business rule) **Pre-condition:** Order placed on partner
site with a customer note **Steps:**
...
**Expected Result:** Customer note orders bypass auto-routing and appear in
manual queue
...
**Priority:** High **Pre-condition:** An order is in the Manual Placement queue
**Steps:**
...
**Priority:** High **Pre-condition:** A floating shelf order is in manual
placement **Steps:**
...
**Expected Result:** Floating shelf orders can be manually placed in WiksHub
with UPS
...
**Priority:** Medium (negative test) **Steps:**
...
**Priority:** Medium **Pre-condition:** A user with "Partner Hub" role exists
**Steps:**
...
- Manual Placement page structure may differ significantly from standard
  WooCommerce pages
- The 367-line page object suggests a complex form — verify all field selectors
  with codegen
- Partner role (restricted Hub access) requires a separate test user with that
  role
