...
async function waitForConnectionSuccess(page, partnerName, timeout = 20000) {
  ...
  const connectedPattern = new RegExp(`Connected Partner:\\s*${escapeRegex(partnerName)}`, 'i');
  ...
  try {
    ...
    const bodySnippet = await page.evaluate(() => {
      ...
      return {
        toast:
          (text.match(/You have successfully connected with partner:[^\n]*/i) || [])[0] || null,
...
async function assertSettingsUpdated(page, timeout = 30000) {
  ...
  try {
    await Promise.race([
      toastLocator.first().waitFor({ state: 'visible', timeout }),
      page.waitForFunction((expected) => (document.body.innerText || '').includes(expected), text, {
        timeout,
      }),
    ]);
  } catch {
...
async function getVisibleNotificationText(page) {
  const selectors = [NOTIFICATION_SELECTOR, '[role="status"]'].join(', ');
