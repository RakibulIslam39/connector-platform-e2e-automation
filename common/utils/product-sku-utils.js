...
function importedProductMatchesExpected(imported, expected, skuPrefix) {
  ...
  if (partnerSku && importedSku === partnerSku.toLowerCase()) {
    return true;
  }
  if (partnerSku && importedSku.includes(partnerSku.toLowerCase())) {
    return true;
  }
  if (importedTitle === normalizedExpectedTitle) {
    return true;
  }
  if (expected.customTitle && importedTitle === normalizeProductTitle(expected.customTitle)) {
    return true;
  }
  if (normalizedCatalogName && importedTitle === normalizedCatalogName) {
    return true;
  }
  if (normalizedCatalogName && importedTitle.includes(normalizedCatalogName)) {
    return true;
  }
  return false;
}
