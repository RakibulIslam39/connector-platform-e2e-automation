...
class WpCliService {
  ...
  _detectMode() {
    if (this.sshHost) return 'ssh';
    if (this.wpPath && process.env.WP_CLI_LOCAL === 'true') return 'local';
    return 'rest-api';
  }
  ...
  _buildCommand(wpCmd) {
    ...
    throw new Error('[WpCliService] Cannot build CLI command in REST API mode — use REST methods instead');
  }
  ...
  async getTaxonomyTerms(taxonomy) {
    ...
    return this._exec(
      `term list ${taxonomy} --fields=term_id,name,slug,count --format=json`
    );
  }
  ...
  async _restGetWcV3Products() {
    ...
    while (pageNum <= totalPages && pageNum <= 50) {
      ...
      const { data: products, totalPages: headerPages, total } = await this._restFetchPaged(url);
  ...
  async _restGetTermsForProduct(postId, taxonomy) {
    ...
    const attr = (product.attributes || []).find(
      (a) => a.slug === taxonomy || `pa_${a.slug}` === taxonomy || a.name?.toLowerCase() === taxonomy.replace(/^pa_/, '').replace(/_/g, ' ')
    );
    if (!attr) return [];
