...
test.describe.serial('Partner Onboarding — Full Flow', () => {
  ...
  test.describe('Scenario 1: Create Partner', () => {
    ...
    test('TC-CP-001: should create a new partner with all required configurations', async ({
      ...
      // ── Step 24b: Clear Connector Hub caches ──────────────────────────────
      // Prevents a stale cached API key (from a previous partner) being read below.
      await test.step('Clear Connector Hub caches so the new API key is fresh', async () => {
  ...
  test.describe('Scenario 2: Partner Site Configuration', () => {
    test('TC-CP-002: should configure connector plugin on partner site using stored API key', async ({
      ...
      await test.step('Input stored API key and submit', async () => {
        ...
        // Strict: the site must connect to the partner we just created. If it
        // connects to a different (e.g. leftover) partner, Scenario 3 would be
        // validating the wrong imported data — fail loudly here instead.
        expect(
          (runtimeState.connectedPartnerName || '').trim(),
          `Connected partner must exactly match the created partner "${runtimeState.partnerName}"`
        ).toBe((runtimeState.partnerName || '').trim());
      });
