...
test.describe.serial('Partner Onboarding — Full Flow', () => {
  ...
  test.describe('Scenario 1: Create Partner', () => {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    test('TC-CP-000: should remove any existing partner for the test website URL', async ({
      partnerCleanupPage,
    }) => {
      ...
      await test.step(`Ensure no partner exists for ${websiteUrl}`, async () => {
    ...
    test('TC-CP-001: should create a new partner with all required configurations', async ({
      partnerFormPage,
    }) => {
      ...
      await test.step('Add ALL available products', async () => {
        ...
        expect
          .soft(count, 'At least 44 products should be added on hub')
      ...
      await test.step('Set Double Curved custom title', async () => {
        ...
        await partnerFormPage.setProductCustomTitle(
          'Double Curved',
          runtimeState.doubleCurvedCustomTitle
        );
      ...
      // ── Step 26: Clear Connector Hub caches ───────────────────────────────
      // The Connector Platform caches the partner→website mapping. Without
      // clearing after creation, Scenario 2's API-key save on the partner site
      // fails CP validation with "partner_website_mismatch" because the Platform
      // still serves stale partner data. Runs in the Hub (admin-bar) context.
      await test.step('Clear all Connector Hub caches so the new partner is live', async () => {
  ...
  test.describe('Scenario 2: Partner Site Configuration', () => {
    test('TC-CP-002: should configure connector plugin on partner site using stored API key', async ({
      ...
      expect(
        runtimeState.partnerName,
        'Partner name must be available from Scenario 1'
      ).toBeTruthy();
  ...
  test.describe('Scenario 3: Imported Product & Attribute Validation', () => {
    ...
    test('TC-CP-003: should validate all imported products exist and match stored configurations', async ({
      connectorSettingsPage,
    }) => {
      ...
      expect(
        runtimeState.doubleCurvedCustomTitle,
        'doubleCurvedCustomTitle must be set'
      ).toBeTruthy();
      expect(
      ...
      // Partner-specific FAQ + Shipping & Returns policy (created in TC-CP-001,
      // Steps 22–23) must appear on this partner's product view page. These are
      // partner-scoped, so any imported product page for this partner shows them.
      await test.step('Validate partner FAQ + Shipping & Returns on the product view page', async () => {
        const products = await wpCli.getProducts();
        const skuCandidates = buildDoubleCurvedSkuCandidates(runtimeState.skuPrefix).map((s) =>
          s.toLowerCase()
        );
        const product =
          products.find((p) =>
        ...
        expect.soft(product, 'A product must exist to open its view page').toBeTruthy();
        if (!product?.ID) {
          return;
        }

        const viewPage = new PartnerProductViewPage(connectorSettingsPage.page);
        await viewPage.openById(product.ID);

        // Titles carry the created values (answer/description bodies render lazily/
        // empty), and are unique random strings — so a visible title is an exact,
        // partner-specific match.
        expect
          .soft(
            await viewPage.hasFaq(runtimeState.faqTitle),
            `Partner FAQ "${runtimeState.faqTitle}" should be visible on the product page`
          )
          .toBe(true);
        expect
          .soft(
            await viewPage.hasShippingPolicy(runtimeState.policyTitle),
            `Partner Shipping & Returns "${runtimeState.policyTitle}" should be visible on the product page`
          )
          .toBe(true);
      });
    });
