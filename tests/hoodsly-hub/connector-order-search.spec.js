...
// // Target order number is read directly from process.env.TEST_ORDER_NUMBER;
// // each test self-skips when it is not set (see test.skip guards below).
...
//   test('Hoodsly Connector link is visible on the hub dashboard', async ({
//     orderManagementPage,
//   }) => {
...
//   test('searching a valid order number returns search results', async ({ orderManagementPage }) => {
...
//   test('clicking an order result opens the order details page', async ({ orderManagementPage }) => {
//     test.skip(!process.env.TEST_ORDER_NUMBER, 'TEST_ORDER_NUMBER env var not set');
//     await orderManagementPage.openConnectorSearch();
...
//   test('Generated SKU is extractable from order details page', async ({ orderManagementPage }) => {
//     test.skip(!process.env.TEST_ORDER_NUMBER, 'TEST_ORDER_NUMBER env var not set');
//     await orderManagementPage.openConnectorSearch();
//     await orderManagementPage.searchOrderNumber(process.env.TEST_ORDER_NUMBER);
//     await orderManagementPage.clickOrderNumber(process.env.TEST_ORDER_NUMBER);
...
//   test('check whether BOL is generated for a shipped order', async ({ orderManagementPage }) => {
...
//   test('order status text is readable from order detail page', async ({ orderManagementPage }) => {
//     test.skip(!process.env.ORDER_FOR_SHOP_ASSIGN, 'ORDER_FOR_SHOP_ASSIGN env var not set');
//     await orderManagementPage.openConnectorSearch();
//     await orderManagementPage.searchOrderNumber(ORDER_FOR_SHOP_ASSIGN);
//     await orderManagementPage.clickOrderNumber(ORDER_FOR_SHOP_ASSIGN);
...
//     console.log(`Order ${process.env.TEST_ORDER_NUMBER} in Manual Placement: ${visible}`);
