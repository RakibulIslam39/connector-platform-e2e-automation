...
const OPTIONAL_VARS = [
  {
    key: 'WP_ADMIN_DISPLAY_NAME',
    description: 'WordPress admin display name — asserted in login validation tests',
  },
  ...
  {
    key: 'PARTNER_SITE_BASE_URL',
    description: 'Partner WordPress site base URL — required for partner-site test suite',
  },
  {
    key: 'TEST_ORDER_NUMBER',
    description: 'WooCommerce/HoodslyHub order number for SKU/BOL/status verification tests',
  },
  {
    key: 'EXPECTED_ORDER_STATUS',
    description: 'Expected WooCommerce order status for cross-system status sync assertions',
  },
  {
    key: 'ORDER_WITH_BOL',
    description: 'HoodslyHub order number known to have a generated BOL + R+L tracking number',
  },
  {
    key: 'ORDER_FOR_SHOP_ASSIGN',
    description: 'HoodslyHub order number to use in shop assignment / reassignment tests',
  },
  {
    key: 'TARGET_SHOP_NAME',
    description: 'Shop name to assign in shop reassignment tests (e.g. Wilkes, WRH)',
  },
  { key: 'WRH_TEST_ORDER_ID', description: 'Order ID to use in WRH Hub order status update tests' },
  {
    key: 'WRH_NEW_ORDER_STATUS',
    description: 'New order status to set in WRH Hub status update tests (e.g. In Production)',
  },
];
