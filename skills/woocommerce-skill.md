...
- Phone number for WooCommerce checkout must be valid US format for BOL
  generation
- WooCommerce stores orders as WordPress posts (post_type=shop_order)
- Custom order statuses (wc-production, wc-finishing) require WooCommerce plugin
  hooks
- Order meta keys: `updated_est_ship_date`, `est_reason_note` updated by CSM via
  Post Meta Creator plugin
