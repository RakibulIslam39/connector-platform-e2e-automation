...
class OrderManagementPage extends BasePage {
  constructor(page) {
    ...
    this.orderStatusButton = page.locator('button').filter({
      hasText: /In Production|Pre-Assembly|Assembly|Sanding|Finishing|Ready To Ship|Shipped|Completed|Canceled|Add Features/i,
    });
  }
  ...
  async openOrderById(orderId) {
    ...
    await this.goto();
    const orderLink = this.page.locator(`a:has-text("#${orderId}"), a[href*="post=${orderId}"]`).first();
  ...
  async searchOrderNumberWithState(orderNumber) {
    ...
    if (!orderText) throw new Error('orderNumber cannot be empty');
    ...
    if (await this._waitForOrderInSearchResult(orderText, 8000)) return true;
    if (await this.isNoOrderResultMessageVisible(1500)) return false;

    // Fallback: click search button
    await this.clickSearchButton();
    if (await this._waitForOrderInSearchResult(orderText, 10000)) return true;
    if (await this.isNoOrderResultMessageVisible(3000)) return false;
    return false;
  }
  ...
  async getGeneratedSkuValue() {
    ...
    if (bodyMatch && bodyMatch[1].trim()) return bodyMatch[1].trim().split('\n')[0].trim();
  ...
  async getOrderStatusText(timeout = 10000) {
    ...
    const ALLOWED = [
      'In Production', 'Pre-Assembly', 'Assembly', 'Sanding', 'Finishing',
      'Ready To Ship', 'Shipped', 'Completed', 'Canceled', 'Add Features',
    ];
    for (const status of ALLOWED) {
      if (text.toLowerCase() === status.toLowerCase()) return status;
      if (new RegExp(`\\b${status}\\b`, 'i').test(text)) return status;
    }
    return text;
  }
  ...
  async waitForShopAssignment(shopName, timeout = 12000) {
    ...
    try {
      ...
      await this.shopNa.first().waitFor({ state: 'hidden', timeout }).catch(() => null);
    }
  }
