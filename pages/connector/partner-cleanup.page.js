...
class PartnerCleanupPage {
  ...
  constructor(page) {
    ...
    this.statusSelect = page.locator('select').filter({ has: page.locator('option', { hasText: 'Trash' }) }).first();
  ...
  async permanentlyDeleteByQuery(query) {
    ...
    while (
      ...
      const remaining =
        ...
        (await this._rowsMatchingQuery(query).count());
      logger.info(
  ...
  async filterByTrash() {
    ...
    const clicked = await this.page.evaluate(() => {
      const el = Array.from(document.querySelectorAll('a, button, li, span'))
        .find(e => e.textContent.trim() === 'Trash');
    ...
    logger.warn('[PartnerCleanupPage] Could not locate Trash filter — proceeding without status change');
  }
  ...
  async ensureNoPartnerWithUrl(websiteUrl) {
    ...
    let permanentlyDeleted = await this.permanentlyDeleteMatchingPartners(websiteUrl);
