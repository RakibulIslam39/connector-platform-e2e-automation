...
class PlatformSettingsPage extends BasePage {
  constructor(page) {
    ...
    this.faqLabelHeader = page
      .locator(PLATFORM_SETTINGS_SELECTORS.FAQ_LABEL_HEADER)
      .filter({ hasText: 'No Label' });
    ...
    this.faqDescriptionEditor = page
      .frameLocator('iframe')
      .locator(PLATFORM_SETTINGS_SELECTORS.FAQ_DESCRIPTION_TINYMCE);
    this.faqSaveBtn = page
      .locator('span')
      .filter({ hasText: /^Save$/ })
      .first();
  ...
  async searchOrderLog({ partnerName, orderId }) {
    ...
    if (await this.isOrderLogPayloadVisible()) {
      return 'found';
    }
    if (await this.isOrderLogErrorVisible()) {
      return 'not-found';
    }
    return 'unknown';
  }
