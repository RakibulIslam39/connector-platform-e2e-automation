...
class ProductManagementPage extends BasePage {
  ...
  async updateProductDescription(productName, descriptionText) {
    ...
    try {
      ...
      const editorFrame = this.page
        .frameLocator('iframe[id*="mce"], iframe[title*="Rich Text Area"], .mce-edit-area iframe')
        .first();
  ...
  async getProductSyncStatus(productName) {
    logger.info(
      `[ProductManagementPage] getProductSyncStatus: not available in SPA for "${productName}"`
    );
    return 'unknown';
  }
}
