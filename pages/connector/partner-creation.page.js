...
class PartnerCreationPage extends BasePage {
  ...
  async _addProductViaModal(productName) {
    ...
    // Confirm modal
    const confirmBtn = this.page.getByRole('button', { name: /^(Add|Confirm|Save|Done)$/i }).last();
  ...
  async deselectProducts(productNames) {
    ...
    await this._openProductsTab();
    for (const name of productNames) {
      const row = this.page.locator('table tbody tr').filter({ hasText: name.trim() }).first();
      try {
  ...
  async _selectAttributeItems(sectionLabel, itemNames) {
    ...
    for (const itemName of itemNames) {
      const row = this.page.locator('table tbody tr').filter({ hasText: itemName }).first();
  ...
  async isPartnerSaved() {
    ...
    try {
      ...
      const errorIndicator = this.page.locator('[class*="error"], [class*="alert-danger"]').first();
