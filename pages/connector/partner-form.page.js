...
class PartnerFormPage extends BasePage {
  ...
  constructor(page) {
    ...
    // ── Admin-bar cache control (Kinsta / Connector Hub) ────────────────────────
    // Rendered as an ARIA `menuitem` (not a link); use role-agnostic text match
    // so it works regardless of markup. Submenu only appears on hover.
   ...
   * Caches" control. Run this after Create Partner so the edit form returns the
   * new partner's freshly-generated API key instead of a cached one.
   */
  async clearAllCaches() {
    logger.info('[PartnerFormPage] Clearing all Connector Hub caches');
    await this.clearCachesToolbar.waitFor({ state: 'visible', timeout: 15000 });
    await this.clearCachesToolbar.hover();

    // Admin-bar submenu may be hover- or click-triggered — handle both.
    ...
    await cacheResponse;
    logger.info('[PartnerFormPage] Connector Hub caches cleared');
  }
  ...
  async readStoredApiKeyForPartner(partnerName) {
    ...
    // Bust the SPA/client cache so the list + edit form refetch from the server.
    ...
    await this.page
      .getByRole('heading', { name: 'Edit Partner' })
    ...
    logger.info(
      `[PartnerFormPage] Read stored API key for ${partnerName}: ${apiKey.substring(0, 8)}...`
    );
    return apiKey;
  }
  ...
  async addAllProducts() {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      const availableBtn = () => this.page.getByRole('button').filter({ hasText: /VEN-/ }).first();
      ...
      while (clickCount < MAX_PRODUCTS) {
        ...
        logger.info(
          `[PartnerFormPage] Adding product: ${parsed.catalogName} (${parsed.catalogSku})`
        );
  ...
  async setProductCustomTitle(productName, customTitle) {
    ...
    const simpleRow = this.page.locator('div, tr, li').filter({ hasText: productName }).last();
    ...
    const titleInput = targetRow
      .locator(
        'input[placeholder*="title" i], input[placeholder*="custom" i], input[placeholder*="name" i], input[type="text"]'
      )
      .first();
    ...
    if (await titleInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      logger.warn(
        `[PartnerFormPage] Custom title input not found for ${productName} — row may not have this field`
      );
    }
  }
  ...
  async _expandAccordion(label) {
    ...
    const isAlreadyExpanded = await this.page
      .evaluate((searchLabel) => {
        ...
        for (const el of allEls) {
          if (el.textContent.trim() !== searchLabel) {
            continue;
          }
          ...
          if (sibling) {
            const hasCheckboxes =
              sibling.querySelectorAll('input[type="checkbox"], [role="checkbox"]').length > 0;
            if (hasCheckboxes) {
              return true;
            }
          }
  ...
  async clickSelectAllColors() {
    ...
    const selectAllCheckbox = this.page.getByRole('checkbox', { name: /select all/i }).first();
  ...
  async fillPrimedCustomPrice(price) {
    ...
    const priceInput = row
      .locator('td')
      .nth(5)
      .locator('input[type="number"], [role="spinbutton"]')
      .first();
  ...
  async addFaq(title, answer) {
    ...
    if (areaVisible) {
      ...
      logger.warn(
        '[PartnerFormPage] Could not fill FAQ answer — no textarea visible after Code mode switch'
      );
    }
  ...
  async addPolicy(policyTitle, policyDescription) {
    ...
    const codeBtnPolicyVisible = await codeBtnPolicy
    ...
    if (descVisible) {
      ...
      logger.warn(
        '[PartnerFormPage] Could not fill policy description — no textarea visible after Code mode switch'
      );
    }
  ...
  async _waitForPartnerCreationResult(timeout = 60000) {
    ...
    while (Date.now() < deadline) {
      ...
      if (notificationText) {
        ...
        if (
          /failed to create partner|website may already exist|already exist/i.test(notificationText)
        ) {
    ...
    throw new Error(
      'Partner creation timed out — still on Add New Partner page with no success signal'
    );
  }
  ...
  async clickCreatePartner() {
    ...
    const toastVisible = await this.page
      .locator(
        '[role="alert"], [role="status"], [class*="toast"], [class*="snackbar"], [class*="notification"]'
      )
    ...
    if (toastVisible) {
      ...
      logger.info(
        '[PartnerFormPage] Partner created — toast auto-dismissed (navigation was the success signal)'
      );
    }
  }

  // ─── Edit existing partner (TC-PC-002) ──────────────────────────────────────
  // SCAFFOLD — selectors marked TODO(codegen) to be confirmed with
  // `npx playwright codegen` against the live Edit Partner page.

  /**
   * Opens the Edit Partner form for an existing partner by name.
   * @param {string} partnerName
   */
  async openPartnerForEdit(searchTerm = null) {
    await this.navigate(CONNECTOR_PATHS.PARTNERS);
    await this.page.reload({ waitUntil: 'domcontentloaded' });

    // `searchTerm` may be a partner name OR the site host (each partner row shows
    // its website URL), so searching by PARTNER_SITE_BASE_URL host resolves the
    // correct partner deterministically.
    if (searchTerm) {
      const search = this.page.locator('input[placeholder="Search partners..."]');
    ...
    const row = searchTerm
      ? this.page.locator('table tbody tr').filter({ hasText: searchTerm }).first()
    ...
    // Resolve the real partner name from Basic Info (needed when the first
    // available partner was opened without a name).
    await this.basicInfoTab.click();
    const resolved = (
      (await this.partnerNameInput.inputValue().catch(() => '')) ||
      searchTerm ||
      ''
    ).trim();
    logger.info(`[PartnerFormPage] Opened Edit Partner: ${resolved}`);
    return resolved;
  }

  /**
   * Adds a single "Available Product" to the partner via the Products tab.
   * Reuses the same inline "Add Products" panel pattern as addAllProducts():
   * the panel exposes a search box and each available product as a <button>
   * whose text contains the product name + SKU.
   * @param {string} productName
   */
  async addProductToPartner(productName) {
    await this._openProductsTab();
    await this.addProductsBtn.click();
    ...
    // The matching available product renders as a clickable button; clicking it
    // adds it to the partner's selected list.
    const productBtn = this.page.getByRole('button').filter({ hasText: productName }).first();
    await productBtn.waitFor({ state: 'visible', timeout: 10000 });
    await productBtn.click();
    logger.info(`[PartnerFormPage] Added product to partner: ${productName}`);

    // Close the inline Add Products panel.
  ...
  /**
   * Clicks Update Partner and returns the visible notice text (expect "Partner updated.").
   * Uses the same native-click fallback as clickCreatePartner because the SPA
   * sometimes ignores synthetic clicks.
   * @returns {Promise<string>}
   */
  async updatePartner() {
    const updateBtn = this.page.getByRole('button', { name: 'Update Partner' }).first();
    ...
    const toast = this.page
      ...
      .filter({ hasText: /Partner updated/i })
      .first();

    let text = await toast.textContent({ timeout: 5000 }).catch(() => null);
    if (!text) {
      // Retry via native DOM click if the synthetic click was swallowed.
      await updateBtn.evaluate((el) => el.click()).catch(() => {});
      text = await toast.textContent({ timeout: 8000 }).catch(() => '');
    }
    const notice = (text || '').trim();
    logger.info(`[PartnerFormPage] Update Partner notice: ${notice}`);
    return notice;
  }
}
