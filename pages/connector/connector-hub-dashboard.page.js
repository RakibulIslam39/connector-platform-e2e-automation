...
class ConnectorHubDashboardPage extends BasePage {
  constructor(page) {
    ...
    this.attributesTab = page
      .locator('header a[href="#/attributes"]')
    ...
    this.partnersByPlatformHeading = page.locator('h3').filter({ hasText: 'Partners by Platform' });
    ...
    this.quickNavigationHeading = page.locator('h3').filter({ hasText: 'Quick Navigation' });
  ...
  async navigateViaSidebarMenu() {
    ...
    await this.page
      .locator(
        '#toplevel_page_connector-hub .wp-submenu a[href="admin.php?page=connector-hub#/dashboard"]'
      )
      .click();
    await this.waitForLoad();
  }
  ...
  async getPlatformData() {
    return this.page.evaluate(() => {
      ...
      return knownPlatforms.map((name) => {
        ...
        if (!nameSpan) {
          return { platform: name, count: null, percentage: null };
        }
