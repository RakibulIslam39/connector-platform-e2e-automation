...
class WrhOrdersPage extends BasePage {
  constructor(page) {
    ...
    this.orderResultItems = page.locator('div.order-meta, div.order-meta a.order-id, a.order-id, table tr');
  ...
  async searchOrderInResult(orderId, timeoutMs = 12000) {
    ...
    while (Date.now() < deadline) {
      ...
      if (await this._isNoResultVisible()) return false;
  ...
  async selectOrderStatusByClicking(orderStatus) {
    ...
    if (!normalized) throw new Error('orderStatus cannot be empty');
    const selectedStatus = this.page.locator(
