...
class PartnerProductEditPage extends BasePage {
  ...
  async openProductEdit(productId) {
    ...
    await this.page.locator('#woocommerce-product-data').waitFor({ state: 'visible', timeout: 15000 });
  }
  ...
  async _readAccordionTable(accordionCollapse) {
    const rows = accordionCollapse.locator('table.widefat tbody tr');
    ...
    for (let i = 0; i < count; i++) {
      ...
      const optionName = ((await cells.nth(0).textContent()) || '').replace(/\s+/g, ' ').trim();
      if (!optionName || /option name/i.test(optionName)) continue;

      let currentPrice = null;
      let customPrice = null;

      const currentText = ((await cells.nth(1).textContent()) || '').trim();
      if (currentText) currentPrice = parsePriceText(currentText);

      const customInput = cells.nth(2).locator('input[type="number"], input[type="text"]').first();
      if (await customInput.isVisible({ timeout: 500 }).catch(() => false)) {
        const val = (await customInput.inputValue()) || '';
        if (val) customPrice = parsePriceText(val);
      }

      parsed.push({ optionName, currentPrice, customPrice });
    }
  ...
  async getPartnerColorNames() {
    ...
    await collapse.locator('table.widefat tbody tr').first()
  ...
  async getPartnerVentilationNames() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (const row of rows) {
        if (!row.optionName || /^none$/i.test(row.optionName)) continue;
  ...
  async getAttributeValues(attributeLabel) {
    ...
    if (normalized.includes('color')) return this.getPartnerColorNames();
    if (normalized.includes('ventilation')) return this.getPartnerVentilationNames();
    if (normalized.includes('trim')) return this.getPartnerTrimNames();
    if (normalized.includes('size')) return this.getPartnerSizeNames();
   ...
   */
  async getPrimedPaintReadyValues() {
    const colorOptions = await this.getColorOptionsPricingTable();
    let primedRow = colorOptions.find((r) => /primed.*paint.*ready/i.test(r.optionName));

    if (!primedRow) {
      const colorRows = await this.getAccordionTableRows('accordion-color');
      primedRow = colorRows.find((r) => /primed.*paint.*ready/i.test(r.optionName));
    }
    ...
    logger.info('[PartnerProductEditPage] Primed/Paint Ready row not found in Custom Price accordions');
