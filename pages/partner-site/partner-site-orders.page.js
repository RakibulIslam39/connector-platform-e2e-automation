...
class PartnerSiteOrdersPage extends BasePage {
  constructor(page) {
    super(page);
    this.woocommerceMenuLink = page.locator('div').filter({ hasText: /^WooCommerce$/ }).first();
  ...
  async getTrackingNumber() {
    ...
    for (const label of candidates) {
      try {
        ...
        if (nextText) return nextText.replace(':', '').trim();
      } catch {
        continue;
      }
    }
  ...
  async getOrderStatus() {
    ...
    if (!status) throw new Error('Order status is empty on partner site order detail page.');
