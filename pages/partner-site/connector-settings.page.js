...
class ConnectorSettingsPage extends BasePage {
  ...
  constructor(page) {
    ...
    this.autoPublishToggle = page
      ...
      .filter({ has: page.locator('..').filter({ hasText: /publish.*automatically|auto.*publish/i }) })
      .first();
    ...
    this.importBtn = page
      ...
      .or(page.locator('input[type="submit"][value="Import"], button').filter({ hasText: /^import$/i }))
      .first();
  }
  ...
  async loginAsPartner() {
    ...
    await this.page.waitForLoadState('domcontentloaded');
    await this.page.waitForURL(/wp-admin/i, { timeout: 20000 });
  ...
  async submitApiKey() {
    ...
    if (!payload.success) {
      const errors = Array.isArray(payload.data) ? payload.data.join('; ') : JSON.stringify(payload.data);
      throw new Error(`[ConnectorSettingsPage] API key save failed: ${errors}`);
    }

    if (payload.data?.name) {
      logger.info(`[ConnectorSettingsPage] API key saved — connected partner: ${payload.data.name}`);
    }
